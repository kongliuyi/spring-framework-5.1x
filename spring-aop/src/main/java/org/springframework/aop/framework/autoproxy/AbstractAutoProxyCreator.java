...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	...
	public Object postProcessBeforeInstantiation(Class<?> beanClass, String beanName) {
		...
		// 这里主要因为 FactoryBean 的 beanName "&"开头
		Object cacheKey = getCacheKey(beanClass, beanName);
	...
	public Object postProcessAfterInitialization(@Nullable Object bean, String beanName) {
		if (bean != null) {
			// 获取当前 bean 的  key，如果 beanName 不为空，则以 beanName 为 key，
			// 如果为 FactoryBean 类型， 前面会还会添加 &符号，如果 beanName 为空， 则以当前的bean 对应的class 为 key
			Object cacheKey = getCacheKey(bean.getClass(), beanName);
			// 判断当前的 bean 是否是正在被代理的 bean，是则不进行封装
			if (this.earlyProxyReferences.remove(cacheKey) != bean) {
				// 如果需要被代理则需要封装成指定的 bean
				return wrapIfNecessary(bean, beanName, cacheKey);
			}
		}
		return bean;
	...
	/**
	 * 为给定的 bean 创建 AOP 代理，实际在这一步中进行切面织入，生成动态代理
	...
	protected Object createProxy(Class<?> beanClass, @Nullable String beanName,
		...
		// beanFactory = DefaultListableBeanFactory implements ConfigurableListableBeanFactory
		if (this.beanFactory instanceof ConfigurableListableBeanFactory) {
			AutoProxyUtils.exposeTargetClass((ConfigurableListableBeanFactory) this.beanFactory, beanName, beanClass);
		}

		// 首先构造一个 ProxyFactory 工厂，此工厂提供方法 getProxy 获取代理对象
		// 但其细节是通过其父类先得到 AopProxy 的工厂，再通过工厂创建 AopProxy，再通过 AopProxy 得到代理对象
